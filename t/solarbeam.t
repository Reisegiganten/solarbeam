...
is_query($sb->build_url(), '/select', wt => 'json');

is_query($sb->build_url({page => 5, rows => 10}), '/select', rows => 10, start => 40, wt => 'json');

is_query($sb->build_url({fq => 'hello*'}), '/select', fq => 'hello*', wt => 'json');

is_query(
  $sb->build_url({fq => ['(foo)', {bar => 1}, ['qux:%qux', qux => \'cool*']]}),
...
is_query(
  $sb->build_url({facet => {field => 'identifier.owner', mincount => 1}}),
...
is_query(
  $sb->build_url({-endpoint => 'terms', terms => {fl => 'artifact.name'}}), '/terms',
  wt         => 'json',
  terms      => 'true',
...
is_query($sbc->build_url({cool => 1}), '/select', wt => 'json', cool => 1, awesome => 1);
